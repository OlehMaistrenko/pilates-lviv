...
GET    /profile/announce/            анонси (майстер-класи/воркшопи; event:int|null —
                                      анонс МОЖЕ бути привʼязаний до тренування)
POST   /profile/announce/{id}/enroll/   запис на анонс БЕЗ event
POST   /profile/announce/{id}/decline/  скасування запису на анонс
```
...
| ---------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Персональна інформація | `GET /profile/info/`                                     | `UserPortalProfile`: `name`, `relation_detail`, `account`, `rules_accepted`. У відповіді API це список (основний профіль + сімейні), але сімейні профілі в проєкті не реалізуються — беремо лише основний запис |
...
**`/profile/announce/` — свідомо ПОЗА цим розділом.** Анонс (`ProfileAnnounce`)
— окрема сутність від тренування (`Event`): заголовок/дата/картинка/опис плюс
опційне поле `event: int|null` — анонс МОЖЕ бути привʼязаний до тренування
(тоді запис іде через звичайний `POST /profile/visit/`, як тренування), а
МОЖЕ не мати event узагалі (воркшоп/семінар без слота в розкладі — тоді
запис іде через `POST /profile/announce/{id}/enroll/`). «Історія занять» у
кабінеті v1 показує тільки `/profile/visit/`; анонси без `event` туди не
потрапляють — клієнт не побачить запис на «чистий» анонс у «Моїх заняттях».
Це той самий шар, що вже є на сайті як «Події» (п.8 структури,
`events.php`/`event-single.php`), і там дані свідомо статичні
(`partials/events-data.php`, майбутній WP CPT) — не з InstaSport. Звести ці
дві речі (кабінетну історію анонсів і публічну сторінку подій) в одне —
поза обсягом v1, окрема задача, якщо знадобиться.
...
5. Три локації (Брюховичі / Чупринки / Сихів) — чи всі три заведені в InstaSport
   як окремі `hall`, чи одна (на діючому сайті адреса одна — розбіжність із
   структурою сайту, див. CLAUDE.md)
6. Чи є **ціни/тарифи** в InstaSport (`public/card_template/`), чи заводити руками
7. **Формат посилання в листі** для `email_login_signup`/`email_reset_password`/
   `user/email_update/` — і що InstaSport дописує в query при редіректі на
   `next_url`. У схемі є тільки «надсилає лист із посиланням», без прикладу
   URL і без окремого `email_verify`/`*_verify` ендпоінта (на відміну від
   phone-флоу, де код підтверджують явно). Без тестового листа від
   InstaSport не розрізнити, чи підтвердження відбувається на сторінці
   самого InstaSport, чи лист веде прямо на наш `next_url` із токеном, який
   нема чим прийняти. **Некритично для v1**: авторизація на проєкті йде за
   телефоном (SMS-код, задокументований наскрізь — див. §2), email —
   лише довідковий контакт у профілі зі своєю кнопкою «Змінити»
   (`partials/modals/contact-change.php`). Якщо ця дірка в документації не
   закриється — зміну email можна прибрати з UI без втрат для основного
   флоу.
