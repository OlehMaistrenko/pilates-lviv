...
// Шаблон детальної сторінки курсу — за зразком training-reformer.php.
...
// Потоки — [дата початку, тривалість і графік, назва, опис, ключ курсу].
// TODO: реальні дати від клієнта; поки показуємо три найближчі як приклад.
$intakes = [
  ['13 жовтня 2026', '8 тижнів · пн, ср — вечір', 'Mat: базовий рівень',
   'Класичний репертуар на маті, анатомія руху, побудова заняття для групи. Стартовий модуль — з нього починають усі.', 'mat-base'],
  ['17 листопада 2026', '10 тижнів · сб, нд — день', 'Reformer: обладнання',
   'Пружинний опір, робота на реформері, Cadillac і Wall Unit. Потрібен закритий Mat або підтверджений досвід викладання.', 'reformer'],
  ['2 лютого 2027', '4 тижні · нд — день', 'Робота зі спеціальними групами',
   'Вагітність і після пологів, біль у спині, відновлення після травми. Модуль для тих, хто вже веде заняття.', 'special'],
];
        ...
        <table>
          <thead>
            <tr>
              ...
              <th scope="col">Годин</th>
              <th scope="col">Що відпрацьовуємо</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">Mat</th>
              <td>64</td>
              <td>Класичний репертуар на маті, анатомія руху, побудова заняття для групи</td>
            </tr>
            <tr>
              <th scope="row">Reformer</th>
              <td>80</td>
              <td>Пружинний опір, робота на реформері, Cadillac і Wall Unit, підбір навантаження</td>
            </tr>
            <tr>
              <th scope="row">Спеціальні групи</th>
              <td>32</td>
              <td>Вагітність і після пологів, біль у спині, відновлення після травми</td>
            </tr>
            <tr>
              <th scope="row">Стажування</th>
              <td>За домовленістю</td>
              <td>Пробні заняття в студії під наглядом викладачки, розбір після кожного</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<!-- ============================================================
     04 · Потоки — .rows: однотипні рядки, тож волосина між ними
     виправдана (CLAUDE.md). Картками .posts не робимо: у потоку
     головне — дата й кнопка, а не кадр.
     ============================================================ -->
<section class="section pt-0">
  <div class="container container--narrow">
    <div class="section-head section-head--split">
      <h2 data-reveal="lines">Найближчі потоки</h2>
      <a class="btn btn--outlined" href="events.php">Усі події</a>
    </div>

    <ul class="rows" data-reveal>
      <?php foreach ($intakes as [$date, $schedule, $title, $note, $key]): ?>
        <li class="rows__item rows__item--pair">
          <div>
            <p class="rows__num"><?= $date ?></p>
            <p class="text--sm text--muted mt-1"><?= $schedule ?></p>
          </div>
          <div>
            <h3 class="rows__title"><?= $title ?></h3>
            <p class="text--sm text--muted mt-2"><?= $note ?></p>
            <button type="button" class="btn btn--outlined btn--sm mt-4"
                    data-modal="callback?course=<?= rawurlencode($title) ?>">Записатись на потік</button>
          </div>
        </li>
      <?php endforeach; ?>
    </ul>
