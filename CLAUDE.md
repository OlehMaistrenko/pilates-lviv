# Заготовка — кодстайл та дизайн-система

## Стек

- Vanilla HTML / CSS / JS — без фреймворків, без білдтулів
- **PHP-інклуди** для спільного лейауту та секцій — сторінки мають розширення `.php`,
  повторювані блоки живуть у `partials/` і підключаються через `include`.
  Сайт треба віддавати PHP-сервером (`php -S localhost:8000`), не `file://`.
- CSS — два файли, за принципом critical/deferred (PageSpeed):
  - `css/main.css` — підключається `<link>` у `<head>`, render-blocking
    навмисно. Токени, reset, глобальні utility (text/spacing/reveal/label/
    section-head), layout (container/section), хедер+nav+мобільне меню/пошук
    (розмітка й тумблери), skip-link.
  - `css/styles.css` — підключається `<link>` у самому кінці `<body>`,
    перед `<script src="js/main.js">`. Усе нижче першого екрана: секції
    сторінок, `.simple-text`, accordion, футер-скелет, повна стилізація
    модалок/мобільного меню/пошуку (вони мають `hidden` у розмітці —
    браузер ховає їх ще до завантаження CSS, тож deferred-стилі не
    спричиняють блимання).
  - Вендорські стилі (Swiper/Mapbox — гейт `$vendor_swiper`/`$vendor_map`
    перед `include footer.php`) підключаються так само в кінці `<body>`,
    **перед** `styles.css`
  - Нову секцію нижче першого екрана — завжди в `styles.css`. У `main.css`
    йде лише те, що видиме одразу на завантаженні (хедер/глобальний
    utility) — за замовчуванням вважай нову секцію deferred, якщо не
    впевнений.
- Один JS-файл: `js/main.js`
- Партіали (хедер, футер, повторювані секції, модалки): `partials/`

---

### Spacing scale

One spacing scale, defined once in the `:root` block of `css/main.css`. Never add a parallel token (`--gap-*`, `--pad-*`, etc.) that duplicates a value the scale already covers — if a component needs "a medium gap," reuse the closest existing step instead of inventing a named alias for it.

- Small, fine-grained steps (button/pill padding, icon gaps) can stay fixed `rem` — a fluid range on a value that's only a few px has no visible effect and isn't worth the complexity.
- Larger, layout-rhythm steps (section gaps, card gaps, margins between blocks) should be fluid (`clamp()`), so mobile doesn't inherit desktop-sized gaps. Interpolate linearly between a mobile size and a desktop size across a min/max viewport range (the [Utopia](https://utopia.fyi/type/calculator/) method) rather than a naive `clamp(min, Xvw, max)` guess — the naive form isn't anchored to real viewport widths and drifts unpredictably between breakpoints.
- The desktop end of the range must equal the step's current/intended desktop value — desktop should render exactly as before; only the mobile floor is new.
- Section-level tokens (container padding, section padding) follow the same fluid-scale principle and already exist — don't reintroduce a separate spacing system alongside them.
- Adding a genuinely new step is fine; adding a same-magnitude token under a different name is the thing to avoid.

## Структура файлів

```
index.php               — головна (порожній каркас: include header + footer)
partials/
  header.php             — <head>, хедер, nav, мобільне меню, пошук
  footer.php             — закриває <main>, футер, модалка-оболонка, main.js
  modals/
    loader.php            — AJAX-ендпоінт, whitelist фрагментів
    example.php           — приклад фрагмента модалки
css/main.css             — critical CSS (<head>): токени, reset, utility, хедер
css/styles.css           — deferred CSS (кінець <body>): секції, модалки, футер
js/main.js               — reveal, motion (Lenis+GSAP), header-state, меню, tabs, accordion
assets/                  — зображення, іконки (icons/sprite.svg)
css/vendor/, js/vendor/  — swiper, mapbox (за гейтом); gsap, lenis (глобально)
api/                     — прокладки до InstaSport (ключ на сервері, не в браузері)
docs/                    — технічні ТЗ (у git); info/ — асети клієнта, у .gitignore
```

Кольорову палітру й шрифти для конкретного проєкту додавай у `:root`
(`css/main.css`) і `@font-face` там само — заготовка навмисно тримає лише
нейтральні `--c-page`/`--c-ink` і `system-ui`.

---

## Кабінет, запис, оплати — [docs/ACCOUNT-WP.md](docs/ACCOUNT-WP.md)

Повне ТЗ: як працює InstaSport API і як писати код зараз, щоб він переїхав на
WP без переписування. Читати цілком перед роботою над кабінетом/записом.

**Під час верстки актуальні три секції** — решта потрібна тільки на бекенді:

- **§10 Запис на тренування** — цільовий UX модалки `booking`: список способів
  оплати (абонемент / рахунок / картка / бронь) залежить від того, чи клієнт
  залогінений і які абонементи підходять під це заняття. Верстати одразу
  двостанною, а не як просту форму.
- **§9 Сторінки кабінету** — які поля реально приходять з API. Не вигадувати
  свої статуси абонемента: `status_detail` і `paid_detail` — **готові
  локалізовані рядки** з InstaSport.
- **§12 Фронтенд** — наявна машинерія модалок (`data-modal`, `?modal=`,
  AJAX-лоадер) переїжджає на WP без змін. Але сабміт форм кабінету треба
  виключити з глобального перехоплювача `.form` у `js/main.js` — інакше
  реальний запит підміниться модалкою подяки.

Три правила, які діють і на верстці:

1. **Токен і `X-API-Key` — ніколи в браузері.** Ніякого `localStorage`, ніяких
   прямих звернень до `instasport.ua` з JS. Тільки свої ендпоінти (§3).
2. **Сторінки кабінету не кешуються** — інакше page cache віддасть кабінет
   одного клієнта іншому (§3, §7.1).
3. **Клієнт кабінету — не `WP_User`.** InstaSport лишається єдиним джерелом
   правди (§4).

---

## Брейкпоінти

```
1080px   планшет / малий десктоп
 768px   мобільний
 560px   дрібні форми/модалки
```

---

### .simple-text (WP rich text)

Блок для довільного HTML з CMS. Підтримує: `p h2 h3 h4 ul ol li blockquote cite figure figcaption img table thead tbody tr th td hr strong em a video iframe`.

Відео-embed — голий `<iframe>`, без обгортки: `aspect-ratio` стоїть
безпосередньо на `.simple-text iframe`.

Для `table` обов'язково обгортати (горизонтальний скрол, якщо колонки не влазять):

```html
<div class="simple-text__table-wrap">
  <table>
    ...
  </table>
</div>
```

---

## CSS — структура файлу

Блоки розділені коментарями:

```css
/* ============================================================
   Назва блоку
   ============================================================ */
```

Порядок блоків у `css/main.css`:

1. Токени `:root`
2. Reset & base
3. Utility (icon, text, spacing, reveal, label, section-head)
4. Layout (container, section)
5. Skip-link
6. Header + main nav

Порядок блоків у `css/styles.css`:

1. Modal overlay, Form, мобільне меню, пошук (deferred — приховані `hidden`)
2. `.simple-text`, Accordion
3. Секції сторінок у порядку появи
4. Footer

**Респонсів — одразу під десктопними правилами свого блоку**, не в одному
загальному `Responsive`-розділі внизу файлу. Кожен `@media` для секції/компонента
йде одразу після її десктопних правил, у межах того самого named-блоку — щоб
усі стилі одного компонента (десктоп + адаптив) читались одним шматком, без
стрибків у кінець файлу. Виняток — спільні utility-брейкпоінти (напр.
`.mt-md-*`), які й так живуть при своєму utility-блоку.

---

## Типографіка — дисципліна

Консистентність важливіша за різноманіття. Жорсткі правила:

- **Розмір тексту** — тільки через токени `--fs-*` (`--fs-xs … --fs-mega`).
  Жодних ad-hoc `rem`/`px` для `font-size`.
- **Розмір і колір довільного тексту** (лід-абзаци, підписи, другорядний текст)
  — через utility-класи `.text--xs/--sm/--body/--lead` (розмір) і
  `.text--muted` (колір), а не власний `font-size`/`color` у block-класі.
  Block-клас лишає собі тільки лейаут (`max-width`, `margin`, `display`).
  Заголовки (H1–H3, `section-head__title` тощо) і mono-лейбли під це правило
  не підпадають — там власні класи.
- **Насиченість** — фіксовані ваги `--fw-reg` (400), `--fw-med` (500),
  `--fw-sb` (600). Скільки саме гарнітур і які ваги вони фізично мають —
  рішення проєкту; не додавай вагу, якої немає у файлі шрифту.
- **Великі заголовки** — `line-height: var(--lh-display)` +
  `letter-spacing: var(--tracking-tight)`.
- **Лейбли/лічильники** — utility-клас `.label`: uppercase,
  `letter-spacing: var(--tracking-mono)`, `--fs-xs`.
- **Гарнітури** — рівно дві ролі: `--font-display` (заголовки, лейбли,
  кнопки) і `--font-body` (тіло, підписи). Заготовка тримає обидві як
  `system-ui` — проєкт підставляє свої `@font-face` в `css/main.css`.

---

## Правила написання коду

- **Класи** — BEM-inspired: `.block`, `.block__element`, `.block--modifier`
- **Нові класи** — тільки якщо немає підходящого існуючого (перевіряй `label`,
  `reveal`, `section-head`, `text`, `container`, `simple-text`, `accordion`,
  `modal-overlay`)
- **Генеричність понад прив'язку до секції.** Компонент, що повторює патерн
  з іншого місця (слайдер-контроли, картка, панель) — генеричний клас
  (`.slider-controls`, не `.team__controls`/`.reviews__controls`), а не
  копія під назвою секції. Прив'язана до секції назва виправдана лише
  тоді, коли розмітка/стилі справді специфічні для цієї секції і ніде
  більше не повторяться.
- **Дизайн-компоненти бренду** (кнопки, картки, плашки, теги тощо) у заготовці
  немає навмисно — додавай їх на конкретному проєкті разом із палітрою і
  шрифтами, а не тягни зі старого сайту.
- **Контейнери** — `.container` (1760px, дефолт), `.container--narrow` (1120px,
  довгі тексти), `.container--full` (без ліміту: герой, CTA, панелі, полотна).
- **`max-width` — на врапері, і тільки в `rem`.** Ширину рядка обмежує
  спільний контейнер (`.container--narrow`) або один врапер колонки, а не
  кожен блок усередині окремо. Не пиши `max-width` на заголовку, ліді,
  абзаці й підписі поруч — від цього блоки в одній колонці отримують різні
  ліві межі й ряд «їде». Одиниця — `rem`, не `ch`: `ch` міряється шириною
  гліфа «0» конкретної гарнітури, тож те саме число дає різну ширину при
  зміні шрифту чи ваги, і поруч із `rem`-шкалою відступів ці величини не
  узгоджуються. Виняток — коли блок справді один у своїй колонці й ширина
  є частиною його форми (напр. лід банера).
- **Hover-стани** — тільки на клікабельних елементах (`a`, `button` тощо).
  Некліковані елементи (звичайні картки, списки, текстові блоки) hover не
  отримують — це фальшивий сигнал інтерактивності.
- **Одиниці — `rem` від 4px і більше.** Будь-яке значення ≥ 4px пишемо в
  `rem` (4px = `0.25rem`, 12px = `0.75rem`, 560px = `35rem`): у `px` воно
  не масштабується разом із системним розміром шрифту, і верстка ламається
  в користувача, який його збільшив. У `px` лишаються тільки величини
  < 4px — волосини й дрібні оптичні поправки (`1px` бордер, `2px`
  `border-radius` плашки, `3px` зсув на hover): там перерахунок у `rem` дає
  дробові значення, які браузер однаково округлює до пікселя, а читається
  це гірше. Брейкпоінти в `@media` — теж `px`: вони міряють вʼюпорт, а не
  текст.
- **Коментарі** — тільки на те, чого не видно з коду: _чому_ так, а не _що_
  тут написано. Коментар, який переказує сусідній рядок, не пишемо взагалі.
  Пишемо, коли є неочевидна причина: обхід баґу вендора, порядок, від якого
  щось залежить, свідома відмова від очевиднішого рішення, зв'язок із кодом
  в іншому файлі. Іменований блок-коментар секції (шапка `/* === Назва === */`)
  під це правило не підпадає — він для навігації.
- **Інлайн-стилі** — тільки для CSS-змінних що варіюються (`style="--tag-c: #b8dee0"`)
- **Зображення** — завжди `loading="lazy"` крім першого above-the-fold (`loading="eager"`)
- **Іконки** — іконка, що повторюється в кількох місцях, не inline-copy-paste
  на кожному місці, а `<symbol>` у `assets/icons/sprite.svg`, підключається
  через `<svg class="icon"><use href="assets/icons/sprite.svg#icon-name"></use></svg>`.
- **Анімації** — тільки через `transform`/`opacity`/`clip-path`, нічого що
  тригерить layout. Прості reveal-и — наявний `[data-reveal]`
  (IntersectionObserver + CSS, дешево для PageSpeed). GSAP ScrollTrigger — лише
  там, де потрібен scrub або pin: `data-anim="words|pin|parallax|zoom|odometer|focus"`.
  Вендори (Lenis/GSAP/ScrollTrigger) підключені глобально в `footer.php`,
  `defer`, без гейта — плавний скрол є на всіх сторінках.
- **JS** — vanilla, IIFE для локальної логіки, вже підключений `main.js` бере на себе все стандартне
- **Нові inline-скрипти** — тільки якщо логіка специфічна для однієї сторінки
- **Семантика** — `<article>` для новин, `<aside>` для сайдбару, `<nav>` для навігацій, `<figure>` для зображень з підписом

!!! не запускай сервер для перевірки. Я перевірятиму сам вручну

---

## Хедер / футер

Не копіювати — підключати через `include 'partials/header.php'` /
`include 'partials/footer.php'`. Активний пункт меню задається змінною `$nav`
**перед** `include` (ключі — по `$primary` у `header.php`); `header.php` сам
проставляє `aria-current="page"`. Меню (`$primary`), лого/бренд, колонки
футера — генеричні заглушки, замінюються на проєкті.

Модалки — спільна оболонка `.modal-overlay` (footer.php) + AJAX-фрагменти
з `partials/modals/*.php`, whitelist у `partials/modals/loader.php`. Тригер —
будь-який елемент з `data-modal="name"` (або `data-modal="name?query=..."`).

Акордеони — нативний `<details>`-подібний патерн через клас `.accordion` +
`.accordion__summary`/`.accordion__body`, стан і ексклюзивність (`data-accordion-group`)
рахує `js/main.js`.

---

# Дизайн-напрямок — Pilates Lviv

Затверджена концепція: «геральдичний спокій». Лого (лев у teaser-позиції —
та сама поза, що й людина на реформері) і брендбук — єдине джерело правди
для кольору й шрифтів, нічого зі старого сайту не тягнемо.

- **Палітра брендбуку без змін**: Moss `#4A4F3C` (і темніший `#3A3E2F`),
  Beige Sand `#EAE5DF`, Burnt Umber `#84412F`, Deep Black. Золото
  (`#C2A15C` → `#DCC58E`) додане як п'ятий колір — воно вже в лого, але як
  фарба в брендбуку не описане.
- **Основний фон сайту — легкий беж** `#EAE5DF` (`--clr-page`), текст —
  Deep Black / темний мох. Сторінка бежева за замовчуванням; секція без
  явного фону лишається бежевою.
- **Зелених секцій — мінімум.** Moss — рідкісний темний акцент, а не
  чергування через одну: герой, футер і максимум один змістовий акцент
  на сторінці. Якщо на макеті більше трьох мохових полів — це помилка.
- **Умбра — тільки двері.** Один умбровий блок на сторінці: запис/розклад.
  Ніде більше — ні в кнопках, ні в тегах, ні в іконках.
- **Золото — акцент, не декор.** Заливка дозволена тільки на `.btn--filled`
  (основна дія: запис, підтвердження) — більше ніде суцільною плямою. Поза
  кнопкою золото лишається як і було: лінія 1px, цифра, лев, підпис.
- **Шрифт — рівно один на весь сайт: `Geologica`** (Google Fonts, варіативна
  вага 200–700, кирилиця повна, включно з Є/І/Ї/Ґ). Замінює одразу обидві
  платні гарнітури брендбуку (`Neue Alte Grotesk` і `Snell Roundhand`) —
  другої, «заголовкової» гарнітури проєкт свідомо не заводить. Обрана за
  формою: пласка вершина «A», кругле «O» — та сама геометрія, що й у
  шрифтовій частині логотипа.
- **Курсиву на сайті немає ніде** — ні `font-style: italic`, ні окремої
  courсивної гарнітури. Ієрархія заголовків (H1/H2 проти H3/лейблів)
  тримається на розмірі, вазі (`--fw-lt` вгорі, `--fw-med`/`--fw-sb` дрібно)
  і трекінгу. Акцентне слово в заголовку — колір (золото), не нахил.
  `est. 2015` ніде окремо не повторюємо скриптом — цей запис уже є
  всередині самого логотипа.
- **Форма** — картки й фото-кадри (портрети, галерея, картки напрямків,
  маркер карти, модалки, дропдауни) мають легке скруглення (`--r-card`,
  1rem) на всіх кутах. Кнопки лишаються пігулкою (`--r-pill`), дрібні
  плашки/теги — гострі (`--r-chip`, 2px). Ніяких тіней/скла/градієнтів.

## Без ознак AI-slop

- **Без кікерів/лейблів за замовчуванням.** Uppercase eyebrow над кожним
  заголовком ("СЕКЦІЯ" / "01 · НАЗВА") — не додавай його автоматично.
  Лейбл виправданий, тільки коли він несе інформацію, якої нема в заголовку
  (категорія тренування, статус) — не як декоративний штамп над кожним блоком.
- **Без зайвих hover-ефектів.** Правило з "Правила написання коду" вище
  діє буквально: якщо елемент не `a`/`button`/клікабельний — він не
  підсвічується, не піднімається, не обводиться при наведенні. Картка
  напрямку без посилання, фото в галереї, статичний блок тексту — hover
  не отримують.
- **Без зайвих бордерів-розділювачів.** Не розділяй кожен блок/секцію
  тонкою лінією за замовчуванням — це найпростіший спосіб зробити сторінку
  схожою на згенеровану презентацію. Розділювач ставиться, коли він
  розв'язує реальну проблему компонування (напр. таблиця/список
  однотипних рядків), не як універсальний "акуратний" кордон навколо
  кожного шматка контенту.
- **Ритм і повітря замість ліній.** Межу між секціями/блоками задавай
  простором (`--sp-*`, зміною фону), а не рамкою. Рамка/волосина — це
  свідомий акцент (напр. золота лінія під заголовком), не структурний
  елемент, що стоїть скрізь.

## Тон копірайту

Без пафосу. Студія говорить конкретикою, а не гаслами — це тон
самого бренду: «Ньюйорківці дуже практичні, тому вони не будуть витрачати
час на те, що не дає результату».

- **Не пишемо**: «Львів тримає поставу», «мистецтво руху», «філософія
  тіла», «подорож до себе», «розкрий свій потенціал». Метафора міста/лева
  живе в логотипі — словами її переказувати не треба.
- **Пишемо**: що саме буде, на чому, скільки і для кого. «Пілатес на
  Cadillac і Reformer», «щоб спина не боліла», «перше заняття —
  знайомство». Заголовок може бути просто фактом.
- Найкращі готові репліки — прямі цитати тренерів із діючого сайту (нижче).
  Вони конкретні й теплі; вигаданий копірайт поруч із ними завжди програє.

## Реальний контент — джерело `pilateslviv.com`

Діючий сайт студії. Беремо звідти фактуру, не вигадуємо свою.

- **Напрямки (реальний каталог, 6):** Пілатес (Springtone) · Йога ·
  Функціональне відновлення · Танці · Навчання · Консультація фізіолога.
  Не Barre/Mat/Pre-postnatal — цього в студії немає.
- **Головна фактична перевага:** «Ми єдина у Львові пілатес-студія, де
  можна позайматись на професійних тренажерах для пілатесу: Cadillac і
  Reformer». Плюс: молодий колектив тренерів, метод Springtone.
- **Тренери з їхніми цитатами** (готовий контент для секції):
  Галина — «Коли мій клієнт каже, що в нього більше не болить спина, — це
  найбільша втіха для мене»; Оксана — «Я допомагаю людям повернути
  здоров'я та радість життя»; Сюзанна — «Ось вже 10 років я займаюсь тим,
  що повертаю людям радість руху»; Ірина — «Повернути людині красиву
  поставу та здорове тіло — це те саме, що повернути віру в себе»;
  Назарій (танці) — «Танець — це спосіб досягнути краси і гармонії,
  володіючи кожним м'язом»; Вікторія (функціональний тренінг) — «Вже
  9 років з його допомогою я тримаю себе у чудовій формі»; Вікторія
  (йога) — «Йога — це не спорт, а скоріше мистецтво».
- **Історія засновниці** (реальна, для секції «Про студію»): Нью-Йорк,
  «За 15 хвилин прогулянки до метро я побачила аж 4» студії пілатесу;
  «Ньюйорківці дуже практичні…»; «Я переконана в тому, що бажання дбати
  про своє тіло є і в нас, українців».
- **Контакти:** +38 (063) 015-05-17 · м. Львів, вул. Б. Романицького, 24а ·
  @pilates_lviv · est. 2015.
- **Розбіжність, яку треба уточнити:** на діючому сайті одна адреса, у
  структурі нижче — три локації (Брюховичі, Чупринки, Сихів). Верстаємо
  під три, контент по двох нових треба отримати від клієнта.
- **Цін на діючому сайті немає** — тарифи запитати окремо.

# Структура вебсайту

## 1. Головна

- Презентаційний банер-слайдер
- Категорії / Типи тренувань
- Записатись / Розклад
- Переваги
- Про студію
- Галерея
- Тренери
- Відгуки
- FAQ
- SEO блок

## 2. Категорії / Типи тренувань

- Список тренувань
  - **Детальна сторінка тренування**
    - Банер
    - Галерея
    - Загальна інформація
    - Переваги та особливості
    - Тренери
    - Галерея
    - CTA для запису
    - Відгуки
    - Відео презентація
    - Записатись на тренування
    - FAQ
    - Інші тренування

## 3. Ціни / Тарифи

## 4. Розклад тренувань

- Вибір студії

## 5. Про студію

- Презентаційний банер з відео
- Загальна інформація
- Історія
- Місія та цінності
- Досягнення
- Про засновницю
- Партнери / Клієнти
- Сертифікати
- Категорії / Типи тренувань

## 6. Локації (Брюховичі, Чупринки, Сихів)

- **Детальна сторінка локації**
  - Презентаційний банер з відео
  - Загальна інформація
  - Карта розташування
  - Контактна інформація + форма зв'язку
  - Тренери на локації
  - Галерея (можна додати 3D-тур, напр. [LapentorPilates](https://app.lapentor.com/sphere/pilates-2))
  - CTA на розклад занять
  - FAQ

## 7. Наша команда / Інструктори

- **Детальна сторінка інструктора**
  - Загальна інформація
  - Сертифікати
  - Які заняття і де проводить
  - Записатись

## 8. Події

- **Детальна сторінка події**

## 9. Навчальний центр (курси)

- Презентаційний банер
- Список подій
- Відео / Текстові відгуки
- Ціни
- FAQ
- Переваги
- Форма запису

## 10. Співпраця

- Презентаційний банер
- Переваги
- Що можемо запропонувати
- Відео / Текстові відгуки
- Форма запису

## 11. Блог

- Список статей
- Сторінка автора

## 12. Питання та відповіді

## 13. Контакти

## 14. Кабінет клієнта

- Персональна інформація
- Баланс
- Історія абонементів
- Історія занять
- Зміна паролю

## 15. Політика конфіденційності

## 16. Сторінка 404
